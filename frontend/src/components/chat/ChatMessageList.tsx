  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6 custom-scrollbar bg-slate-50/50">
      {messages.map((msg) => {
        ...
        :[msg.content];
        const msgKey = msg.id || `${msg.role}-${messages.indexOf(msg)}`;
        return (
        <div key={msgKey} className={`flex items-start gap-3 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"} animate-in fade-in slide-in-from-bottom-2`}>
          {/* アイコン */}
          ...
          <div className={`flex flex-col max-w-[75%] ${msg.role === "user" ? "items-end" : "items-start"} space-y-2`}>
            ...
            style={{ animationDelay: `${messages.indexOf(msg) * 0.15}s` }}
            >
              {section.trim()}
            </div>
            ))}
