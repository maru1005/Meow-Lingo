...
export function Sidebar() {
  ...
  // fetchHistory は chat/page.tsx が責任を持つので、ここでは取り出さない
  const {
    isSidebarOpen,
    toggleSidebar,
    history,
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-emerald-100 transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="p-5 flex flex-col h-full">
          ...
          <div className="mt-8 flex-1 overflow-y-auto space-y-2">
            ...
            {!user && (
              <p className="text-xs text-emerald-400 ml-2 italic">
                ログインして履歴を表示
              </p>
            )}
            ...
            {history.map((item, idx) => (
              <div
                key={item.conversation_id || `fallback-${idx}`}
